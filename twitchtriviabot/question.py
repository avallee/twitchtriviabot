...
class Question(object):
    ...
    def __init__(self, row, session_config, music_mode=False):
        ...
        if music_mode:
            ...
            self.question = str(
                row['question'][0]).upper() + row['question'][1:]
        ...
        if self.session_config['mode'] == 'poll2':
            try:
                ...
                logging.debug(
                    'ERROR ON PARSING QUESTION WITH ANSWER/ANSWER2, CHECK TRIVIA SOURCE')
        ...
        self.skipped = False
        # this is an ordered list used for polling multiple answers as they
        # come in
        self.answered_user_list = []
        ...
        if self.session_config['mode'] != 'poll2':
            try:
                for answer in self.answers:
                    # strict new matching
                    if bool(re.match('\\b%s\\b' %
                                     answer, cleanmessage, re.IGNORECASE)):
            ...
            try:
                # strict new matching
                if bool(re.match('\\b%s\\b' %
                                 self.answers[0], cleanmessage, re.IGNORECASE)):
                    logging.debug('Answer recognized: %s' % self.answers[0])
                    return True, 1  # answer_slot 1
                # strict new matching
                elif bool(re.match('\\b%s\\b' % self.answers[1], cleanmessage, re.IGNORECASE)):
                    logging.debug('Answer recognized: %s' % self.answers[0])
                    return True, 2  # answer_slot 1
                return False, None
            except BaseException:
                logging.debug('Try/except error, no match on %s to %s' %
                              (cleanmessage, answer))
                return False, None

    def answer_string(self, user, trivia_num):
        return '%s answers question #%s correctly! The answer is ** %s ** with a %s point value. %s has %s points!' % (
            user.username, trivia_num, self.answers[0], self.point_value, user.username, user.points)

    def answer_string_poll(self, user, point_val, trivia_num):
        if self.answered_user_list_remaining:
            return '%s answers question #%s first correctly! The answer is ** %s ** with a %s point value. %s has %s points! Others who answered: %s' % (
                user.username, trivia_num, self.answers[0], point_val, user.username, user.points, ', '.join(self.answered_user_list_remaining))
        else:
            return '%s answers question #%s first correctly! The answer is ** %s ** with a %s point value. %s has %s points!' % (
                user.username, trivia_num, self.answers[0], point_val, user.username, user.points)

    def answer_string_poll2(self, user, point_val, trivia_num):
        if self.answered_user_list_remaining:
            return '%s answers question (2nd category) #%s first correctly! The answer is ** %s ** with a %s point value. %s has %s points! Others who answered: %s' % (
                user.username, trivia_num, self.answers[1], point_val, user.username, user.points, ', '.join(self.answered_user_list_remaining))
        else:
            return '%s answers question (2nd category) #%s first correctly! The answer is ** %s ** with a %s point value. %s has %s points!' % (
                user.username, trivia_num, self.answers[1], point_val, user.username, user.points)
    ...
    def check_actions(self):
        time_since_question_asked = (
            datetime.datetime.now() - self.question_time_start).seconds

        if (time_since_question_asked >=
                self.session_config['hint_time1']) and not self.hint1_asked:
    ...
    def find_poll_score(self):
        """for 'poll'/'poll2' mode, this takes the answered_user_list and
        ...
        try:
            if self.answered_user_list:
                ...
                self.point_dict = {
                    self.answered_user_list.pop(): self.point_value + 2}
                self.answered_user_list_remaining = [
                    i.username for i in self.answered_user_list]
                try:
                    self.point_dict[self.answered_user_list.pop()
                                    ] = self.point_value + 1
                except BaseException:
                    pass
                try:
            ...
            logging.debug('Error on find_poll_score: %s' %
                          traceback.print_exc())
        ...
        if self.session_config['mode'] == 'poll2':
            ...
            try:
                if self.answered_user_list2:
                    ...
                    self.point_dict2 = {
                        self.answered_user_list2.pop(): self.point_value + 2}
                    self.answered_user_list_remaining2 = [
                        i.username for i in self.answered_user_list2]
                    try:
                        self.point_dict2[self.answered_user_list2.pop(
                        )] = self.point_value + 1
                    except BaseException:
                        pass
                    try:
