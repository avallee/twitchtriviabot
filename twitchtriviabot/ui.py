...
from PyQt5.QtWidgets import QLabel, QPushButton, QApplication, QMainWindow, QMessageBox, QWidget
...
class MainWindow(QWidget):
    ...
    def __init__(self, version):
        ...
        self.icon.addPixmap(QtGui.QPixmap('ico.ico'),
        ...
        self.title_bar_text = QLabel(
            'Twitch Trivia Bot version %s' % self.version, self.window)
        self.title_bar_text.setGeometry(QtCore.QRect(0, 0, 320, 20))
        self.title_bar_text.setAlignment(
        ...
        self.connect_status = QLabel(
            self.bot_status_dict['connect_status'], self.window)
        ...
        if self.BORDER:
            self.trivia_status_label.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')

        self.trivia_status = QLabel(
            self.bot_status_dict['trivia_status'], self.window)
        ...
        if self.BORDER:
            self.trivia_status.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')
        ...
        if self.BORDER:
            self.category_variable_text.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')
        ...
        if self.BORDER:
            self.question_variable_text.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')

        self.question_no_variable_text = QLabel('', self.window)
        self.question_no_variable_text.setGeometry(
            QtCore.QRect(0, 120, 80, 20))
        self.question_no_variable_text.setAlignment(QtCore.Qt.AlignLeft)
        if self.BORDER:
            self.question_no_variable_text.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')
        ...
        if self.BORDER:
            self.top3_variable_text.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')
        ...
        if self.BORDER:
            self.answer_variable_text.setStyleSheet(
                'border:2px solid rgb(255,255,255); ')
        ...
        self.footer_bar_text = QLabel(
            'Developed by @cleartonic  |  twitch.tv/cleartonic', self.window)
        self.footer_bar_text.setGeometry(QtCore.QRect(0, 380, 320, 20))
        self.footer_bar_text.setAlignment(
            QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter)
        ...
        if self.tb:
            ...
            if self.tb.trivia_active:
                ...
                self.top3_variable_text.setText(
                    self.tb.active_session.check_top_3())
                if str(self.tb.active_session.session_config['music_mode']
                       ) == 'true' or self.tb.active_session.session_config['music_mode'] == True:
                    self.trivia_startq_button.setVisible(True)
                else:
                    self.trivia_skip_button.setVisible(True)
                if self.tb.active_session.session_config[
                        'mode'] == 'poll' or self.tb.active_session.session_config['mode'] == 'poll2':
                ...
                if self.tb.active_session.questionasked:
                    ...
                    self.category_variable_text.setText(
                        self.tb.active_session.active_question.category)
                    self.question_variable_text.setText(
                        self.tb.active_session.active_question.question)
                    self.answer_variable_text.setText(
                        ', '.join(self.tb.active_session.active_question.answers))
                    self.question_no_variable_text.setText(
                        self.tb.active_session.report_question_numbers())
                else:
                    self.category_variable_text.setVisible(False)
                    self.question_variable_text.setVisible(False)
                    self.answer_variable_text.setVisible(False)
    ...
    def skip_question(self):
        if self.tb.active_session.trivia_active:
            try:
                ...
                logging.debug('Error on skipping question : %s' %
                              traceback.print_exc())
    ...
    def start_question(self):
        if self.tb.active_session.trivia_active:
            try:
                ...
                logging.debug('Error on starting question : %s' %
                              traceback.print_exc())
    ...
    def end_question(self):
        if self.tb.active_session.trivia_active:
            try:
                ...
                logging.debug('Error on ending question : %s' %
                              traceback.print_exc())
