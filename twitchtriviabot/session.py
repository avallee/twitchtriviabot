...
class Session(object):
    def __init__(self, cb, trivia_config):
        ...
        self.cb = cb
        # Dictionary holding user scores, kept in '!' and loaded/created upon
        # trivia. [1,2,3] 1: Session score 2: Total trivia points 3: Total wins
        self.userscores = {}
        ...
        # Time when the last question was asked (used for relative time length
        # for hints/skip)
        self.questionasked_time = 0
        ...
        try:
            if self.session_config['music_mode']:
                ...
                with open(os.path.join('config', self.session_config['file_name']), 'r') as f:
                    data = f.read()
                data = csv.reader(data.splitlines(
                ), quotechar='"', delimiter=',', quoting=csv.QUOTE_ALL, skipinitialspace=True)
                self.ts = {}
                for idx, i in enumerate(data):
                    category, question, answer, answer2, creator = i
                    self.ts[idx] = {'category': category, 'question': question,
                                    'answer': answer, 'answer2': answer2, 'creator': creator}

                if self.session_config['mode'] == 'poll2':
                    logging.debug(
                        'Mode set to poll2, only taking questions with valid answer/answer2...')
                    valid_keys = [
                        i for i in self.ts if self.ts[i]['answer2'] != '']
                    new_ts = {}
                    for i in valid_keys:
                        new_ts[i] = self.ts[i]
                    self.ts = new_ts
                ...
                if self.tsrows < self.session_config['question_count']:
                    ...
                    logging.debug(
                        "Warning: Trivia questions for session exceeds trivia set's population. Setting session equal to max questions.")

                logging.debug('Creating session set data. Population %s, session %s' % (
                    self.tsrows, self.session_config['question_count']))
                chosen_idx = random.sample(list(self.ts.keys()), int(
                    self.session_config['question_count']))
            ...
            logging.debug(
                'Error on data load. Check trivia set and make sure file_name matches in config, and that file matches columns/headers correctly\nError code:\n>> %s' % e)
            self.valid = False

        if str(self.session_config['output']).lower(
        ) == 'true' or self.session_config['output'] == True:
            for i in ['1_place_username', '1_place_score', '2_place_username',
                      '2_place_score', '3_place_username', '3_place_score', 'scoreboard']:
                with open(self.get_config_path('scores', '%s.txt' % i), 'w') as f:
                    f.write('')

    def get_config_path(self, *args):
        return os.path.join(
            *itertools.chain((self.session_config['path_dir'],), args))
        ...
        if self.session_config['music_mode']:
            try:
                ...
                self.active_question = Question(
                    [artist, track], self.session_config, music_mode=True)
                self.active_question.activate_question(
                    self.bonus_round, int(self.session_config['question_bonusvalue']))
                self.questionasked = True
                self.cb.send_message('Question %s: %s' % (
                    self.questionno, self.active_question.question_string))
                logging.debug(self.active_question)
            except Exception as e:
                logging.debug(
                    'Error on calling next question, ending trivia...%s' % str(e))
            ...
            try:
                ...
                if self.session_config['length'] == 'infinite':
                    # add the question back, to the start of the index (so its
                    # not popped next)
                ...
                logging.debug(
                    'Error on calling next question, ending trivia...')
                self.force_end_of_trivia()
    ...
    def question_answered(self, user, answer_slot=None):
        if self.session_config['mode'] == 'single':
            ...
            self.cb.send_message(
                self.active_question.answer_string(user, self.questionno))
            self.questionno += 1
            self.questionasked = False
            ...
            if self.questionno < self.question_count + 1:
                try:
                    ...
                    logging.debug('Error on question call %s' %
                                  traceback.print_exc())
            else:
                self.force_end_of_trivia()
    ...
    def output_session_variables(self):
        try:
            ...
            logging.debug(
                'Error on output session variables method (score txt files) %s' % traceback.print_exc())

    def check_user(self, username):
        """This checks the username and returns the User object that
        matches."""
    ...
    def check_user_score(self, user, from_trivia_bot=False):
        if not from_trivia_bot:
            if user.validate_message_time():
                self.cb.send_message('User %s has %s points.' %
                                     (user.username, user.points))
        else:
            self.cb.send_message('User %s had %s points last game.' % (
                user.username, user.points))
    ...
    def check_end_of_trivia(self, end_trivia_flag=False):
        if (self.questionno >
                self.question_count and self.trivia_active) or end_trivia_flag:
            self.write_question_variable()
            ...
            if self.winners:
                try:
                    if len(self.winners) > 1:
                        winners_string = ', '.join(
                            [i.username for i in self.winners])
                        self.cb.send_message(
                            'Trivia is over - a tie between %s for %s points!' % (winners_string, self.winners[0].points))
                    else:
                        winner = self.winners[0]
                        self.cb.send_message(
                            'Trivia is over - %s wins with %s points!' % (winner.username, winner.points))
                except BaseException:
                    self.cb.send_message(
                        'Error on calculating winners: %s' % (self.winners))
            else:
    ...
    def skip_question(self):
        try:
            self.cb.send_message(
                'Question being skipped. The answer was ** %s **.' % self.active_question.answers[0])
            self.active_question.active = False
            self.answered_questions.append(self.active_question)
            self.questionno += 1
            self.questionasked = False
    ...
    def start_question(self):
        try:
            if not self.active_question.active:
                ...
                logging.debug(
                    'Question already active, ignoring Start Q command')
        except BaseException:
            logging.debug('Error on start question %s' % traceback.print_exc())

    def end_question(self):
        try:
            self.active_question.question_time_start = datetime.datetime(
                1990, 1, 1)
    ...
    def manage_poll_question(self):
        ...
        if adj_time < cur_time and self.active_question.active:
            ...
            if self.active_question.point_dict:
                ...
                self.answered_questions.append(self.active_question)
                self.cb.send_message(self.active_question.answer_string_poll(
                    first_user, self.active_question.point_dict[first_user], self.questionno))
            else:
                self.cb.send_message(
                    'Question not answered in time. The answer was ** %s **.' % self.active_question.answers[0])
            ...
            if self.active_question.session_config['mode'] == 'poll2':
                ...
                if self.active_question.point_dict2:
                    try:
                        first_user = list(
                            self.active_question.point_dict2.keys())[0]
                        for user in self.active_question.point_dict2:
                            user.points += self.active_question.point_dict2[user]
                        time.sleep(.5)
                        self.cb.send_message(self.active_question.answer_string_poll2(
                            first_user, self.active_question.point_dict2[first_user], self.questionno))
                    except BaseException:
                        logging.debug(
                            'Error on second question poll, ignoring.')
                else:
                    self.cb.send_message(
                        '2nd category question not answered in time. The answer was ** %s **.' % self.active_question.answers[1])
                ...
                if bonus_users:
                    ...
                    if len(bonus_users) > 3:
                        self.cb.send_message(
                            'More than 3 players answered both prompts correctly, yielding +2 bonus points!')
                    else:
                        self.cb.send_message(
                            '%s answered both prompts correctly, yielding +2 bonus points!' % ', '.join([user.username]))
            self.questionno += 1
            self.questionasked = False

            # if music_mode is True, then do nothing after a question is
            # answered
            if self.session_config['music_mode']:
                pass
    ...
    def check_actions(self):
        try:
            # first check the question object by seeing if a hint or timeout
            # needs to occur
    ...
    def toggle_bonus_mode(self):
        if self.bonus_round:
            ...
            self.cb.send_message(
                'Bonus round disabled. Trivia questions worth 1 point.')
        else:
            self.bonus_round = True
            self.active_question.point_value = int(
                self.session_config['question_bonusvalue'])
            self.cb.send_message('Bonus round is active! Bonus questions are worth ** %s ** points!' %
                                 (self.session_config['question_bonusvalue']))

    def handle_session_message(self, username, message):
        """This is the main function that decides what to do based on the
        latest message that came in For the SESSION."""
        self.check_end_of_trivia()
        if message:
            logging.debug('Session vars trivia_active: %s questionasked: %s' % (
                self.trivia_active, self.questionasked))
            ...
            if self.trivia_active:
                if self.questionasked:
                    if self.session_config['mode'] == 'poll2':
                        match, answer_slot = self.active_question.check_match(
                            message)
                        if match:
