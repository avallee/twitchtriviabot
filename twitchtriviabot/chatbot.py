    ...
    def __init__(self, auth_config):
        ...
        try:
            ...
            try:
                ...
                self.s.connect(
                    (self.bot_config['host'], self.bot_config['port']))
                self.s.send('PASS {}\r\n'.format(
                    self.bot_config['pass']).encode('utf-8'))
                self.s.send('NICK {}\r\n'.format(
                    self.bot_config['nick']).encode('utf-8'))
                self.s.send('JOIN #{}\r\n'.format(
                    self.bot_config['chan']).encode('utf-8'))
                time.sleep(1)
                ...
                self.s.send('NICK {}\r\n'.format(
                    self.bot_config['nick']).encode('utf-8'))
                self.s.send('JOIN #{}\r\n'.format(
                    self.bot_config['chan'].lower()).encode('utf-8'))
                time.sleep(1)
                self.send_message(self.infomessage)
                self.s.setblocking(0)
        except Exception as e:
            logging.debug(
                'Connection failed. Check config settings and reload bot.\nError code:\n%s' % str(e))
            self.valid = False
        logging.debug('Finished setting up Chat Bot.')

    # Chat message sender func
    def send_message(self, msg):
        answermsg = ':' + self.bot_config['nick'] + '!' + self.bot_config['nick'] + '@' + \
            self.bot_config['nick'] + '.tmi.twitch.tv PRIVMSG #' + \
            self.bot_config['chan'] + ' : ' + msg + '\r\n'
        encoding = 'utf-8'
        try:
...
#            logging.debug("Response:\n%s" % response)
            if response == 'PING :tmi.twitch.tv\r\n':
                ...
                if username == self.bot_config['nick']:
                    pass
                else:
                    message = re.compile(
                        r'^:\w+!\w+@\w+\.tmi\.twitch\.tv PRIVMSG #\w+ :').sub('', response)
                    cleanmessage = re.sub(
                        r'\s +', '', message, flags=re.UNICODE).replace('\n', '').replace('\r', '')
