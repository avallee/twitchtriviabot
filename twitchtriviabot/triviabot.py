...
class TriviaBot(object):
    ...
    def __init__(self):
        ...
        if self.valid:
            try:
                ...
                logging.debug(
                    'Failure to connect to Twitch chat. Check auth config and retry')
                self.valid = False
            ...
            self.admins = [i.strip()
                           for i in self.trivia_config['admins'].split(',')]

            logging.debug('Finished setting up Trivia Bot.')
        else:
            logging.debug(
                'Invalid setup - please check trivia_config.yml file')
        ...
        for k, v in temp_config.items():
            if k == 'filename':
                ...
                if not isinstance(v, int):
                    self.error_msg = 'Config error: Error with %s -> %s not being an integer' % (
                        k, v)
                    logging.debug(self.error_msg)
                    self.valid = False
            elif k == 'mode':
    ...
    def validate_auth_config(self, temp_config):
        for k, v in temp_config.items():
            if k == 'host':
                if v != 'irc.twitch.tv':
                    logging.debug(
                        'Config issue: Host name %s is not default irc.twitch.tv - change at discretion only' % v)
            elif k == 'port':
                if int(v) != 6667:
                    logging.debug(
                        'Config issue: Port %s is not default 6667 - change at discretion only' % v)
            elif k == 'nick':
                ...
                if '#' in v or not isinstance(v, str):
    ...
    def start_session(self, start_new_override=True):
        ...
        if not self.trivia_active:
            # if there's already a session, ignore, unless from command
            if not self.active_session or start_new_override:
            ...
            try:
                if self.trivia_config['length'] == 'infinite':
                    self.cb.send_message('Trivia has begun! Infinite question mode. Trivia will start in %s seconds.' % (
                        self.active_session.session_config['question_delay']))
                else:
                    self.cb.send_message('Trivia has begun! Question Count: %s. Trivia will start in %s seconds.' % (
                        self.active_session.question_count, self.active_session.session_config['question_delay']))
                time.sleep(
                    self.active_session.session_config['question_delay'])
    ...
    def load_trivia_session(self):
        ...
        if not self.trivia_active:
            ...
            self.cb.send_message(
                'Latest trivia session loaded. Beginning trivia...')
            self.start_session(False)

        else:
            if self.trivia_active:
                logging.debug(
                    'Trivia session active, cannot load during session.')
            else:
    ...
    def handle_triviabot_message(self, username, message):
        ...
        latest message that came in For the TRIVIA BOT, primarily used for
        commands."""
        if message:
    ...
    def check_active_session_score(self, username):
        """If there's an active session that has not yet been replaced
        (meaning, the last active session after a game is over), this will
        allow users to call their scores."""
        if self.active_session is not None and not self.active_session.trivia_active:
            user = self.active_session.check_user(username)
            # anti spam measure
            if user.validate_message_time():
                if user:
                    self.active_session.check_user_score(
                        user, from_trivia_bot=True)
                else:
                    self.cb.send_message(
                        '%s had no points in the last game.' % (username))
    ...
    def main_loop(self, command_line_mode=True):
        """The main loop is always running from the start While trivia is not
        active, it will check only handle_triviabot_message for incoming
        messages While trivia is active, it will delegate to the active
        session."""
        ...
        if command_line_mode:
            ...
            while self.valid:
                ...
                if iternum % 300 == 0:
                    try:
                        logging.debug('Iternum %s : trivia_active %s active_session.trivia_active %s' % (
                            iternum, self.trivia_active, self.active_session.trivia_active))
                    except BaseException:
                        logging.debug('Iternum %s : %s' %
                                      (iternum, self.trivia_active))
                ...
                if self.active_session.trivia_active:
                    ...
                    if message:
                        if not self.trivia_active:  # when a session is NOT running
                            self.handle_triviabot_message(
                                username, clean_message)

                    if self.trivia_active:  # when a session is running
                        # check every iteration if trivia is active or not, to
                        # set the trivia bot to be inactive
                        if not self.active_session.trivia_active:
                            logging.debug(
                                'Setting trivia to inactive based on active_session...')
                            self.trivia_active = False

                time.sleep(1 / 120)
            logging.debug('Trivia bot no longer valid, ending program.')
        else:
            # when not in command line mode, the window program will handle the
            # looping
            if self.active_session.trivia_active:
                self.handle_active_session()
            else:
                ...
                if self.trivia_active:  # when a session is running
                    # check every iteration if trivia is active or not, to set
                    # the trivia bot to be inactive
                    if not self.active_session.trivia_active:
                        logging.debug(
                            'Setting trivia to inactive based on active_session...')
                        self.trivia_active = False
