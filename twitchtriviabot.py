...
logging.basicConfig(format='%(asctime)s %(message)s',
                    ...
                    filename=os.path.join(
                        THIS_FILEPATH, 'config', 'output_log.log'),
                    filemode='a',
                    level=logging.DEBUG)

VERSION_NUM = '2.1.0'

AUTH_CONFIG_PATH = os.path.abspath(os.path.join(
    THIS_FILEPATH, 'config', 'auth_config.yml'))
TRIVIA_CONFIG_PATH = os.path.abspath(os.path.join(
    THIS_FILEPATH, 'config', 'trivia_config.yml'))
...
if not os.path.exists(AUTH_CONFIG_PATH):
    shutil.copyfile(
        os.path.abspath(os.path.join(
            THIS_FILEPATH, 'init_config', 'auth_config.yml')),
        AUTH_CONFIG_PATH)
if not os.path.exists(TRIVIA_CONFIG_PATH):
    shutil.copyfile(
        os.path.abspath(os.path.join(
            THIS_FILEPATH, 'init_config', 'trivia_config.yml')),
        TRIVIA_CONFIG_PATH)
if not os.path.exists(os.path.abspath(
        os.path.join(THIS_FILEPATH, 'config', 'scores'))):
    os.makedirs(os.path.abspath(os.path.join(
        THIS_FILEPATH, 'config', 'scores')))
if not os.path.exists(os.path.abspath(
        os.path.join(THIS_FILEPATH, 'config', 'music'))):
    os.makedirs(os.path.abspath(os.path.join(
        THIS_FILEPATH, 'config', 'music')))
